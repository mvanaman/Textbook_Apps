...
ui <- fluidPage(
    ...
    # this renders the output element defined in the server
    plotOutput("my_plot")
    )
server <- function(input, output) {
    # This tells R to create an output element - a ggplot, in this case. 
    # which gets sent back to ui 
    output$my_plot <- renderPlot({
        ggplot(iris, aes(x = Petal.Length, y = Sepal.Length)) +
            geom_smooth() +
            labs(
                x = "Willingness to Attend Femininist Punk Rock Shows",
                y = "Potential Points Scored With Kat"
                ) +
            theme_minimal()
    })
}
shinyApp(ui = ui, server = server, options = list(height = 525, width = 400))
```
...
ui <- fluidPage(
  fluidRow(
    column(4, 
      ...
      numericInput("slope", label = "Slope", value = 0, min = -100),
      numericInput("y_intercept", label = "Y-Intercept", value = 0, min = -100)
    )
  ),
  fluidRow(
    column(9, plotOutput("scatterplot"))
